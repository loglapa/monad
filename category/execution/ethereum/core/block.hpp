...
struct BlockHeader
{
    ...
    // Monad does not implement BALs. If EIP-7843 is active, we carry
    // an all-zero BAL hash to maintain parity with Ethereum's RLP format.
    std::optional<bytes32_t> block_access_list_hash{std::nullopt}; // EIP-7928
...
static_assert(sizeof(BlockHeader) == 808);
static_assert(alignof(BlockHeader) == 8);

static_assert(sizeof(Block) == 888);
